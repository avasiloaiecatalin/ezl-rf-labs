...
public class FileUtils 
{
	...
	protected static void writeLearningSetToFile(String fileName, String[][] trainingSet)
	{
		// first create the byte array to be written
		StringBuilder stringBuilder = new StringBuilder();
		for(int n = 0; n < trainingSet.length; n++) //for each row
		{
			//for each column
			 for(int p = 0; p < trainingSet[n].length; p++) 
			 {
				//append to the output string
				 stringBuilder.append(trainingSet[n][p]+"");
				 //if this is not the last row element
			      if(p < trainingSet[n].length - 1)
			      {
			    	  //then add separator
